...
class MigrateFriendsACL implements AsynchronousUpgrade {
	...
	protected function preprareMigration() {
	...
	protected function addFriendsToACLs() {
		...
		$query = "
			...
			AND r.guid_two NOT IN (select subacl.user_guid FROM {$dbprefix}access_collection_membership subacl where subacl.user_guid = acl.owner_guid)
		";
